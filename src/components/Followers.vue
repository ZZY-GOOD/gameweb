<template>
  <section class="panel">
    <div class="list-wrap">
      ...
      <ul>
        <li v-for="u in followersList" :key="u.id" class="row">
          ...
          <button v-if="!isFollowing(u.name)" class="btn small" @click="follow(u.name)">关注</button>
        </li>
      </ul>
...
import { store, followersOf, followUser, isFollowing as _isFollowing } from '../store';
const followersList = computed(() => followersOf(store.user?.name));
